...
HttpData::HttpData(shared_ptr<EventLoop> loop_,int fd_):Channel(loop_,fd_){
  error=false;
...
void HttpData::handleRead(){
  do {
    ...
    if (state == STATE_RECV_BODY) {
      ...
      if (static_cast<int>(inBuffer.size()) < content_length) break;
      state = STATE_ANALYSIS;
    }
  ...
  if (!error) {
    ...
    if (!error && state == STATE_FINISH) {
      this->reset();
      if (inBuffer.size() > 0) {
        if (connectionState != H_DISCONNECTING) handleRead();
      }
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
  ...
  body_buff += "<hr><em> CaoDuanxin's Web Server</em>\n</body></html>";

  header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
  header_buff += "Content-Type: text/html\r\n";
  header_buff += "Connection: Close\r\n";
  header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
  header_buff += "Server: CaoDuanxin's Web Server\r\n";
  header_buff += "\r\n";
  // 错误处理不考虑writen不完的情况
...
void HttpData::handleConn() {
  ...
  if (!error && connectionState == H_CONNECTED) {
    if (events != 0) {
      ...
      if (keepAlive) timeout = DEFAULT_KEEP_ALIVE_TIME;
      ...
      events |= EPOLLET;
      loop->updatePoller(shared_from_this(), timeout);
...
void HttpData::handleClose() {
  ...
  loop->removeFromPoller(shared_from_this());
}
...
AnalysisState HttpData::analysisRequest() {
  if (method == METHOD_POST) {
    ...
    if (fileName == "favicon.ico") {
      ...
      header += "Server: CaoDuanxin's Web Server\r\n";
    ...
    if (method == METHOD_HEAD) return ANALYSIS_SUCCESS;
...
void HttpData::newEvent() {
  ...
  loop->addToPoller(shared_from_this(), DEFAULT_EXPIRED_TIME);
}
void HttpData::linkTimer(std::shared_ptr<TimerNode> mtimer) {
    timer= mtimer;
}
shared_ptr<EventLoop> HttpData::getLoop() { return loop; }